...
export function SEOHead({ game, settings }: SEOHeadProps) {
  useEffect(() => {
    ...
    if (game) {
      ...
      title = game.seoTitle || `${game.title} - Download APK & Claim ${game.bonus} Bonus | ${siteName}`;
      description = game.seoDescription || `Get the official ${game.title} APK download. Claim your exclusive ${game.bonus} Signup Welcome Bonus. Rated ${game.rating || '4.9'}/5 stars. Safe, secure, and fast direct installation.`;
      keywords = game.seoKeywords || `${game.title.toLowerCase()}, ${game.title.toLowerCase()} apk, download ${game.title.toLowerCase()}, ${game.title.toLowerCase()} bonus, ${game.category.toLowerCase()} games, verified rummy app`;
      canonical = game.canonicalUrl || `${canonicalBase}?app=${slug}`;
      ogTitle = game.seoTitle || `${game.title} APK Download - Welcome Bonus ${game.bonus}`;
      ogDesc = game.seoDescription || `Install ${game.title} directly using Play Zone 777's verified link and claim an instant ${game.bonus} Welcome Reward on registration. Scan cleared safe app.`;
      if (game.logoUrl) {
    ...
    if (game) {
      ...
      const gameSlug = game.slug || game.title.toLowerCase().replace(/[^a-z0-9]+/g, '-');
