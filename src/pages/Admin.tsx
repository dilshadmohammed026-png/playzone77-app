                      ...
                      {settings ? (
                        <div className="space-y-4">
                          ...
                          <div className="border-t border-green-900/30 pt-4 mt-4">
                            <h4 className="text-xs font-bold text-amber-500 uppercase tracking-wider mb-3">Custom Ad Network Slots (Adsterra, Monetag, etc.)</h4>
                            
                            <div className="space-y-4">
                              <div>
                                <label className="block text-xs font-bold text-green-100/60 uppercase tracking-wider mb-1.5">Responsive Banner Ad Code (320x50 / 728x90)</label>
                                <textarea 
                                  rows={3}
                                  value={settings.adCodeBanner || ''}
                                  onChange={(e) => setSettings({ ...settings, adCodeBanner: e.target.value })}
                                  placeholder="Paste custom Banner ad HTML/JS snippet here"
                                  className="w-full bg-[#05130b] border border-green-900 rounded-xl p-3 text-xs text-green-50 focus:border-amber-500/50 outline-none font-mono"
                                />
                                <p className="text-[10px] text-green-100/40 mt-1">Used for top, middle, and bottom responsive slots.</p>
                              </div>

                              <div>
                                <label className="block text-xs font-bold text-green-100/60 uppercase tracking-wider mb-1.5">Right Sidebar Ad Code (Desktop Banner/Rectangle)</label>
                                <textarea 
                                  rows={3}
                                  value={settings.adCodeSidebar || ''}
                                  onChange={(e) => setSettings({ ...settings, adCodeSidebar: e.target.value })}
                                  placeholder="Paste custom Sidebar ad HTML/JS snippet here"
                                  className="w-full bg-[#05130b] border border-green-900 rounded-xl p-3 text-xs text-green-50 focus:border-amber-500/50 outline-none font-mono"
                                />
                                <p className="text-[10px] text-green-100/40 mt-1">Displays in the desktop right sidebar layout.</p>
                              </div>

                              <div>
                                <label className="block text-xs font-bold text-green-100/60 uppercase tracking-wider mb-1.5">Sticky Bottom Mobile Banner Ad Code (320x50)</label>
                                <textarea 
                                  rows={3}
                                  value={settings.adCodeStickyBanner || ''}
                                  onChange={(e) => setSettings({ ...settings, adCodeStickyBanner: e.target.value })}
                                  placeholder="Paste custom Sticky Mobile Banner HTML/JS snippet here"
                                  className="w-full bg-[#05130b] border border-green-900 rounded-xl p-3 text-xs text-green-50 focus:border-amber-500/50 outline-none font-mono"
                                />
                                <p className="text-[10px] text-green-100/40 mt-1">Sticky banner floating at the bottom edge of mobile screens.</p>
                              </div>

                              <div>
                                <label className="block text-xs font-bold text-green-100/60 uppercase tracking-wider mb-1.5">Interstitial Ad Code (5-Sec Skippable Overlay)</label>
                                <textarea 
                                  rows={3}
                                  value={settings.adCodeInterstitial || ''}
                                  onChange={(e) => setSettings({ ...settings, adCodeInterstitial: e.target.value })}
                                  placeholder="Paste custom Interstitial/Popunder/Direct Link ad script here"
                                  className="w-full bg-[#05130b] border border-green-900 rounded-xl p-3 text-xs text-green-50 focus:border-amber-500/50 outline-none font-mono"
                                />
                                <p className="text-[10px] text-green-100/40 mt-1">Shown in a 5-second skippable dialog when Details is clicked.</p>
                              </div>
                            </div>
                          </div>
                          ...
                          >
                            {isSavingSettings ? 'Saving Settings...' : 'Save Ad Settings'}
                          </button>
                        </div>
                      ) : (
                        <div className="py-12 text-center text-sm text-green-100/40">Loading settings...</div>
            ...
            <>
              ...
              {(() => {
                ...
                const handleDownloadXmlSitemap = () => {
                  ...
                  const categories = ['Rummy', 'Slots', 'Spin', 'Arcade', 'Games', 'VIP', 'Casino', 'Betting', 'YONO'];
              ...
              {editingGame ? (
                ...
                <form onSubmit={saveEditedGame} className="bg-[#071c10] border border-amber-500/50 rounded-2xl p-6 shadow-xl relative overflow-hidden mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {/* Game-level SEO Settings */}
                    <div className="md:col-span-2 border-t border-green-800/60 pt-6 mt-4">
                      <h3 className="text-sm font-bold text-amber-500 uppercase tracking-wider mb-4 flex items-center gap-2">
                        Game-Specific SEO Configuration (Optional)
                      </h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="block text-xs font-bold text-green-100/50 uppercase tracking-wider mb-2">SEO Custom Title</label>
                          <input 
                            type="text" 
                            value={editingGame.seoTitle || ''}
                            placeholder="e.g. Download ABC Rummy - Claim Exclusive ₹51 Welcome Bonus"
                            onChange={(e) => setEditingGame({...editingGame, seoTitle: e.target.value})}
                            className="w-full bg-[#05130b] border border-green-800 focus:border-amber-500/50 focus:ring-0 rounded-lg px-4 py-3 text-sm text-white placeholder-green-100/20"
                          />
                        </div>
                        <div>
                          <label className="block text-xs font-bold text-green-100/50 uppercase tracking-wider mb-2">Custom SEO URL Slug</label>
                          <input 
                            type="text" 
                            value={editingGame.slug || ''}
                            placeholder="e.g. abc-rummy-pro"
                            onChange={(e) => setEditingGame({...editingGame, slug: e.target.value})}
                            className="w-full bg-[#05130b] border border-green-800 focus:border-amber-500/50 focus:ring-0 rounded-lg px-4 py-3 text-sm text-white placeholder-green-100/20"
                          />
                        </div>
                        <div className="md:col-span-2">
                          <label className="block text-xs font-bold text-green-100/50 uppercase tracking-wider mb-2">SEO Custom Description</label>
                          <textarea
                            rows={2}
                            value={editingGame.seoDescription || ''}
                            placeholder="e.g. Download the official, safe ABC Rummy APK to get an instant registration gift of ₹51..."
                            onChange={(e) => setEditingGame({...editingGame, seoDescription: e.target.value})}
                            className="w-full bg-[#05130b] border border-green-800 focus:border-amber-500/50 focus:ring-0 rounded-lg px-4 py-3 text-sm text-white placeholder-green-100/20"
                          ></textarea>
                        </div>
                        <div>
                          <label className="block text-xs font-bold text-green-100/50 uppercase tracking-wider mb-2">SEO Custom Keywords</label>
                          <input 
                            type="text" 
                            value={editingGame.seoKeywords || ''}
                            placeholder="e.g. abc rummy, abc rummy apk download, free registration bonus"
                            onChange={(e) => setEditingGame({...editingGame, seoKeywords: e.target.value})}
                            className="w-full bg-[#05130b] border border-green-800 focus:border-amber-500/50 focus:ring-0 rounded-lg px-4 py-3 text-sm text-white placeholder-green-100/20"
                          />
                        </div>
                        <div>
                          <label className="block text-xs font-bold text-green-100/50 uppercase tracking-wider mb-2">SEO Custom Canonical URL</label>
                          <input 
                            type="text" 
                            value={editingGame.canonicalUrl || ''}
                            placeholder="e.g. https://playzone777.com/abc-rummy"
                            onChange={(e) => setEditingGame({...editingGame, canonicalUrl: e.target.value})}
                            className="w-full bg-[#05130b] border border-green-800 focus:border-amber-500/50 focus:ring-0 rounded-lg px-4 py-3 text-sm text-white placeholder-green-100/20"
                          />
                        </div>
                      </div>
