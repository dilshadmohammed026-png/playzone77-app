...
import { getGames, Game, trackEvent, getSettings, AppSettings, trackWebsiteView, trackWebsiteReferralClick, trackBannerClick, createReport, getReports, LinkReport, trackAdEvent, subscribeSettings, subscribeGames } from '../lib/store';
...
const filterCategories = ['All', 'Rummy', 'Slots', 'Spin', 'Arcade', 'Games', 'VIP', 'Casino', 'YONO'];
...
export function Home() {
  ...
  useEffect(() => {
    ...
    const unsubscribeGames = subscribeGames((data) => {
      setGames(data);
    });
  ...
  useEffect(() => {
    ...
    if (appIdStr && games.length > 0) {
      ...
      let matchedGame = games.find(g => g.id === appId);
      if (!matchedGame) {
        matchedGame = games.find(g => {
          const gameSlug = g.slug || g.title.toLowerCase().replace(/[^a-z0-9]+/g, '-');
          return gameSlug === appIdStr;
        });
      }
      if (matchedGame) {
  ...
  useEffect(() => {
    ...
    // Dynamic URL update for deep link SEO mapping
    const params = new URLSearchParams(window.location.search);
    if (selectedDetailGame) {
      const slugValue = selectedDetailGame.slug || selectedDetailGame.title.toLowerCase().replace(/[^a-z0-9]+/g, '-');
      if (params.get('app') !== slugValue) {
        params.set('app', slugValue);
  ...
  const handleDetailsClick = async (game: Game) => {
    ...
    const interstitialCode = settings?.adCodeInterstitial?.trim() || settings?.adsenseDetailsCode?.trim();
    const isAdAvailable = settings?.adsenseEnabled && !!interstitialCode;
  ...
  return (
    <div className="min-h-screen bg-[#05130b] text-green-50 font-sans selection:bg-amber-500/30 selection:text-amber-200">
      ...
      <main className="pt-20">
        ...
        {/* TOP AD ZONE */}
        {settings?.adsenseEnabled && (settings?.adCodeBanner || settings?.adsenseTopCode) && (
          <div className="max-w-7xl mx-auto px-4 lg:px-8 pt-4">
            <AdZone code={settings.adCodeBanner || settings.adsenseTopCode} enabled={settings.adsenseEnabled} adUnit="top_banner" />
          </div>
        )}
        ...
        <section className="py-16 relative z-10 px-4">
          <div className="max-w-7xl mx-auto">
            <h2 className="sr-only">Rummy, Slots, and Casino APK Games Directory</h2>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
              ...
              {filteredGames.length > 0 ? filteredGames.flatMap((game, index) => {
                       ...
                       <div className="w-[85px] h-[85px] shrink-0 rounded-[20px] bg-gradient-to-br from-[#0c311c] to-[#05130b] p-0.5 shadow-inner border border-amber-500/30 group-hover:border-amber-400 transition-colors">
                         <div className="w-full h-full rounded-[18px] flex items-center justify-center bg-[url('https://grainy-gradients.vercel.app/noise.svg')] bg-opacity-10 relative overflow-hidden">
                           ...
                           <div className="relative z-10 w-full h-full flex items-center justify-center">
                             {game.logoUrl ? (
                               <img src={game.logoUrl} alt={`${game.title} logo - download official apk`} className="w-[90%] h-[90%] object-cover rounded-[14px]" referrerPolicy="no-referrer" loading="lazy" />
                             ) : (
                ...
                if (settings?.adsenseEnabled && (settings.adCodeBanner || settings.adsenseMiddleCode) && (index + 1) % 3 === 0) {
                  elements.push(
                    <div key={`homepage-middle-ad-${index}`} className="col-span-full py-4 text-center">
                      <AdZone code={settings.adCodeBanner || settings.adsenseMiddleCode} enabled={settings.adsenseEnabled} adUnit="middle_banner" />
                    </div>
                  );
                }
              ...
              <div className="hidden lg:block lg:col-span-1 sticky top-24">
                <div className="bg-[#071c10] border border-green-800 rounded-3xl p-5 shadow-lg">
                  <h4 className="text-sm font-bold text-amber-500 uppercase tracking-wider mb-4 flex items-center gap-2">
                    <Crown size={16} /> Sponsored Ads
                  </h4>
                  <p className="text-xs text-green-100/50 mb-4 leading-relaxed">
                    Check out top verified and tested high reward games. Click details or download now.
                  </p>
                  
                  {/* Right Sidebar Ad Slot */}
                  {settings?.adsenseEnabled && (settings?.adCodeSidebar || settings?.adsenseMiddleCode) && (
                    <AdZone 
                      code={settings.adCodeSidebar || settings.adsenseMiddleCode} 
                      enabled={settings.adsenseEnabled} 
                      adUnit="sidebar_banner" 
                    />
                  )}
                </div>
              </div>
        ...
        {/* BOTTOM AD ZONE */}
        {settings?.adsenseEnabled && (settings?.adCodeBanner || settings?.adsenseBottomCode) && (
          <div className="max-w-7xl mx-auto px-4 lg:px-8 pb-10">
            <AdZone code={settings.adCodeBanner || settings.adsenseBottomCode} enabled={settings.adsenseEnabled} adUnit="footer_banner" />
          </div>
        )}
            ...
            <div className="relative h-44 sm:h-56 bg-gradient-to-r from-emerald-950 to-amber-950 shrink-0">
              {selectedDetailGame.bannerUrl ? (
                <img 
                  ...
                  alt={`${selectedDetailGame.title} banner advertisement`} 
                ...
                <div className="w-full h-full rounded-[20px] flex items-center justify-center bg-[url('https://grainy-gradients.vercel.app/noise.svg')] bg-opacity-10 relative overflow-hidden">
                  <div className="relative z-10 w-full h-full flex items-center justify-center">
                    {selectedDetailGame.logoUrl ? (
                      <img src={selectedDetailGame.logoUrl} alt={`${selectedDetailGame.title} game icon logo`} className="w-[90%] h-[90%] object-cover rounded-[16px]" referrerPolicy="no-referrer" loading="lazy" />
                    ) : (
      ...
      <InterstitialAd 
        ...
        code={settings?.adCodeInterstitial || settings?.adsenseDetailsCode}
        enabled={settings?.adsenseEnabled !== false}
        adUnit={interstitialAdUnit}
      />

      {/* Sticky Bottom Mobile Banner Ad */}
      <StickyAd 
        position="mobile-bottom" 
        code={settings?.adCodeStickyBanner || settings?.adsenseBottomCode} 
        enabled={settings?.adsenseEnabled !== false} 
      />
    </div>
  );
